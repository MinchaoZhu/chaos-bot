...
cp -R "\${PACKAGE_DIR}/frontend" "\${RELEASE_ROOT}/"
...
export CHAOS_BOT_FRONTEND_DIST="\${RELEASE_ROOT}/frontend"
exec "\${RELEASE_ROOT}/bin/chaos-bot-backend" "\$@"
LAUNCHER

chmod +x "\${LAUNCHER_PATH}" "\${RELEASE_ROOT}/bin/chaos-bot-backend"
echo "installed chaos-bot ${release_version} to \${RELEASE_ROOT}"
echo "launcher available at \${LAUNCHER_PATH}"
EOF

chmod +x "${staging_dir}/install.sh" "${staging_dir}/bin/chaos-bot-backend"
